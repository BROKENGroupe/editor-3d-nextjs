...
export function AcousticEditor({ points, onChange }: Props) {
  const handleChange = (index: number, field: keyof AcousticPoint, value: string) => {
    const updated = [...points];
    updated[index] = {
      ...updated[index],
      [field]: field === "id" ? value : parseFloat(value),
    };
    onChange(updated);
  };
  ...
  return (
    <div className="space-y-4">
      {points.map((point, index) => (
        <div key={point.id} className="grid grid-cols-[repeat(4,1fr)_auto] gap-2 items-center">
          <Input
            type="number"
            value={point.x}
